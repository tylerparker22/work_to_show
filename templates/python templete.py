...
# %% manipulate if col a has mult unique var
# descriptive stats only numeric columns 
df.loc[df['Item'] == item_name, df.columns[1:]].describe()

# %%  loop through unique names in a column 
...
# %% loop through descriptive stats
# loop through unique names in a column 
years = df.columns[1:]  # columns to the right
...
# %% clean data if look like piviot table
...
# Convert 'Value' to numeric (in case there are non-numeric entries)
df['Value'] = pd.to_numeric(df['Value'], errors='coerce')

# Drop rows where Value is NaN (optional, if needed)
df_clean = df.dropna(subset=['Value'])

# Descriptive stats per Year
desc_stats = df_clean.groupby('Year')['Value'].describe()
print(desc_stats)
